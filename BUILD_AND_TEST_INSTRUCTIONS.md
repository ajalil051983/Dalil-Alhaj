...
```
No assemblies found in '/data/user/0/com.companyname.dalilalhaj/files/.__override__/x86_64'
```

## Solution Already Applied ?
1. **AndroidManifest.xml** - Added queries for Appium server packages
2. **DalilAlHaj.csproj** - Already has `<EmbedAssembliesIntoApk>true</EmbedAssembliesIntoApk>` 
...
1. In Solution Explorer, **right-click** on the `DalilAlHaj` project
...
```powershell
msbuild "D:\Ai workspace\Dalil Alhaj\DalilAlHaj\DalilAlHaj.csproj" /t:Build /p:TargetFramework=net10.0-android /p:Configuration=Debug
```

**Option C - Using dotnet CLI (if above don't work):**
```powershell
dotnet publish "D:\Ai workspace\Dalil Alhaj\DalilAlHaj\DalilAlHaj.csproj" -f net10.0-android -c Debug
```

### Step 2: Verify the APK was Created
Run this command to check:
```powershell
Test-Path "D:\Ai workspace\Dalil Alhaj\DalilAlHaj\bin\Debug\net10.0-android\com.companyname.dalilalhaj-Signed.apk"
```
...
```powershell
dotnet test "D:\Ai workspace\Dalil Alhaj\DalilAlHaj.UITests\DalilAlHaj.UITests.csproj" --filter "FullyQualifiedName~Categories_ShouldLoadAndDisplay"
```
...
## Files Modified
- ? `DalilAlHaj\Platforms\Android\AndroidManifest.xml`
- ? `DalilAlHaj.UITests\AppiumSetup.cs`
- ? `DalilAlHaj\DalilAlHaj.csproj` (already had the fix)
